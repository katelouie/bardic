...
class TestRenderContent:
    ...
    def test_expression_token(self):
        ...
        content, _, _ = renderer.render_content([{"type": "expression", "code": "name"}])
        assert content == "Alice"

    def test_expression_with_format_spec(self):
        renderer, _ = _make_renderer(state={"price": 9.99})
        content, _, _ = renderer.render_content([{"type": "expression", "code": "price:.2f"}])
        assert content == "9.99"

    def test_expression_error_shows_debug(self):
        renderer, _ = _make_renderer()
        content, _, _ = renderer.render_content([{"type": "expression", "code": "undefined_var"}])
...
class TestIsChoiceAvailable:
    ...
    def test_unconditional_choice_available(self):
        renderer, _ = _make_renderer()
        assert renderer.is_choice_available({"text": "Go", "target": "Next"}, "Start") is True

    def test_true_condition(self):
        renderer, _ = _make_renderer(state={"has_key": True})
        assert (
            renderer.is_choice_available(
    ...
    def test_one_time_choice_hidden_after_use(self):
        renderer, _ = _make_renderer(used_choices={"Room:Look around:Room"})
        assert (
            renderer.is_choice_available(
