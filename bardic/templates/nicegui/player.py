...
def show_story_select():
    ...
    for story_file in sorted(story_files):
        ...
        with (
            ui.card()
            .classes("w-full cursor-pointer hover:shadow-lg")
            .on("click", lambda s=story_id: start_story(s))
        ):
    ...
    if saves:
        ...
        for save in saves:
            ...
            with (
                ui.card()
                .classes("w-full cursor-pointer hover:shadow-lg")
                .on("click", lambda s=save["save_id"]: load_save(s))
            ):
                ui.label(save["name"]).classes("text-lg font-bold")
                ui.label(f"{save['story_id']} - {timestamp.strftime('%Y-%m-%d %H:%M')}").classes(
...
def show_player():
    ...
    with ui.row().classes("w-full justify-between items-center mb-4"):
        ...
        with ui.row().classes("gap-2"):
            ...
            ui.button("Return to Menu", on_click=lambda: navigate_to("story_select")).classes(
                "bg-gray-500"
            )
    ...
    if output.choices:
        ...
        for i, choice in enumerate(output.choices):
            ui.button(choice["text"], on_click=lambda idx=i: make_choice(idx)).classes(
                "w-full mb-2 text-left bg-purple-600 hover:bg-purple-700"
            )
    else:
...
def show_save_dialog():
    ...
    with ui.dialog() as dialog, ui.card().classes("w-96"):
        ...
        save_name_input = ui.input(label="Save Name", placeholder="My Save").classes("w-full mb-4")

        with ui.row().classes("w-full justify-end gap-2"):
            ui.button("Cancel", on_click=dialog.close).classes("bg-gray-500")
            ui.button("Save", on_click=lambda: save_game(save_name_input.value, dialog)).classes(
                "bg-blue-500"
            )
...
def save_game(name: str, dialog):
    ...
    save_manager.save_game(name=name, engine_state=engine.save_state(), story_id=current_story_id)
    dialog.close()
